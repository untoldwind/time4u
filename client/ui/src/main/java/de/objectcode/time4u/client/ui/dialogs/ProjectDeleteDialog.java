...
public class ProjectDeleteDialog extends Dialog
{
  ...
  public ProjectDeleteDialog(final IShellProvider shellProvider, final Project project)
  {
    super(shellProvider);
     ...
     * 
     * searcher.setProject(project);
     * 
     * try { Activator.getDefault().getRepository().getWorkItemStore().iterate(searcher.getFilter(), searcher);
     * 
     * m_results = searcher.getResults(); } catch (final Exception e) { Activator.getDefault().log(e); }
     */
  }
  ...
  {
    parent.getShell().setText("Project delete");
    ...
    warningLabel.setText("If you delete Project '" + m_project.getName()
        + "' the following workitems will become invalid");
